...
桌宠入睡（约 15 分钟无事件）后再等 `SLEEP` 配置的分钟数（默认 5，0=关闭）即息屏：背光真关断、面板发 DISPOFF 关驱动、CPU 降到 80MHz、WiFi 断开改为每 5 分钟开一个约 15 秒的联网窗口（窗口内轮询一次快照后即断网）。本地触摸同样要静默满超时；30 分钟没有任何成功轮询（服务没跑/断网/未配网）也会兜底息屏。唤醒：触摸屏幕、BOOT 键、串口任意命令、长按 PWR，或恢复编码（桌宠离开 sleeping）自动亮屏——息屏期间靠联网窗口发现事件，**最多约 5 分钟延迟**，300ms 淡入。
