...
├── config.*        NVS 配置 + 串口命令
├── net.*           WiFi / 每 1s 轮询快照（直连配置的 PC IP）
...
桌宠入睡（约 15 分钟无事件）后再等 `SLEEP` 配置的分钟数（默认 5，0=关闭）即息屏：背光真关断、WiFi 断开改为每 5 分钟开一个约 15 秒的联网窗口（窗口内轮询一次快照后即断网）。本地触摸同样要静默满超时；30 分钟没有任何成功轮询（服务没跑/断网/未配网）也会兜底息屏。唤醒：触摸屏幕、BOOT 键、串口任意命令、插上 USB、长按 PWR，或恢复编码（桌宠离开 sleeping）自动亮屏——息屏期间靠联网窗口发现事件，**最多约 5 分钟延迟**，300ms 淡入。

两个已踩过的坑，别再踩：不能给面板发 DISPOFF（AXS15231B 显示与触摸一体，关显示会挂掉触摸 I2C）；不能息屏降频（Arduino 的 `setCpuFrequencyMhz` 切频会复位 APB 外设配置，I2C 触摸直接失联）。两者都会连锁饿死 LVGL 锁、令全部唤醒源失效。

醒着时轮询失败有指数退避（2s、4s、…封顶 60s），成功即恢复 1s，避免中转服务关机后每秒硬凿连接超时。

## 低电保护

板子没有电量计，但持续监测 EMA 电压：**未插 USB 且电压低于 3.30V 持续 2 分钟 → 走正常关机流程断电**（USB 插入永不触发；电压回升到 3.50V 以上才会重新武装），防止锂电池过放。
