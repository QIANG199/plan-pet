...
- **PWR 键**：长按约 3 秒触发**关机倒计时**（左侧卡片先出现圆形进度环，满圈后 3·2·1 倒计时，期间再按一次 PWR 取消），走完才断电黑屏；USB 插着永不关机
- **开机动画**：上电循环播放桌宠动画，首批数据到达后淡入桌面（点按可跳过）
- **息屏**：桌宠入睡后再等 5 分钟（串口 `SLEEP` 可调，0=关闭）自动熄屏省电护屏；触摸、BOOT 键、串口命令或恢复编码都会亮屏。wifi 图标变**琥珀**色 = 中转服务失联（约 15 秒无成功快照，屏上是旧数据），红色 = WiFi 断开
- **电量**：电压估算；充电中百分比冻结在插线瞬间的读数（插拔读数一致），拔线后十几秒内平滑恢复
